...
/mob/living/carbon/human/Life()
	...
	if(istype(loc, /turf/unsimulated/floor/river))//People who are on fire go out.
		ExtinguishMob()
...
/mob/living/carbon/proc/check_drowning()
	if(lying && istype(loc, /turf/unsimulated/floor/river/deep))
...
/mob/living/carbon/human/rejuvenate()
	...
	shock_stage = 0
	if(SSjobs.GetJobByTitle(job)?.is_barbarians_team && !(src in SSwarfare.barbarians.team)) //BARBARIANS
		SSwarfare.barbarians.team.Add(src)
	if(SSjobs.GetJobByTitle(job)?.is_romans_team && !(src in SSwarfare.romans.team)) //ROMANS
		SSwarfare.romans.team.Add(src)
	..()
